   ...
   if ((Get-Service -DisplayName 'Cluster Service').status -eq "running" )
       { 
            try
               ...
               $sev=Stop-Service -DisplayName 'Cluster Service' -ErrorAction stop
               
               $sev.Status -eq "STOPPED"

               Write-Host " SUCCESS: Cluster Service is stopped on $global:Computer" 
               }
               ...
               Catch [System.Exception]
               {
		           Write-Host "FAILURE"
		           Write-Host "  EXCEPTION:" $_
		           Log-Cleanup-and-Exit "Not able to stop the cluster service on $global:Computer" 11
                }
        }
